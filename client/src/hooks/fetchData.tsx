...
export function useFetchData(url: string, { defaultState, notNull, noLoad }: FetchDataOptions) {

  const [data, setData] = useState(defaultState)
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<Response | null | unknown>(null)

  // create a ref to the notNull array so that it doesn't need to be a dependency
  const notNullRef = useRef<Array<any> | undefined>();
  ...
  // DELETE function
  async function deleteData(urlArray: string | Array<string>) {
    const method = 'DELETE'
    try {
      setIsLoading(true)
      // check if any of the notNull values are null or undefined
  ...
  async function createRefreshData(data: any) {
    try {
  ...
  async function updateRefreshData(data: any) {
    try {
  ...
  async function deleteRefreshData(data: any) {
    try {
