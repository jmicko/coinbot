...
function Graph(props: GraphProps) {
  ...
  const maxSize = setupOptions.sizeCurve === 'curve'
    ...
    : setupMinSize * 2;
  const currentPrice = Number(setupOptions.currentPrice);
  ...
  const drawGraphBoundary = useCallback((canvas: HTMLCanvasElement) => {
  ...
  const drawPriceTicker = useCallback((canvas: HTMLCanvasElement) => {
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
  ...
  const drawDataPoints = useCallback((canvas: HTMLCanvasElement) => {
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
  ...
  const drawTicks = useCallback((canvas: HTMLCanvasElement) => {
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
    const {
      paddingX,
  ...
  const drawAxis = useCallback((canvas: HTMLCanvasElement) => {
  ...
  function getGraphDimensions(canvas: HTMLCanvasElement) {
  ...
  const drawToolTip = useCallback((canvas: HTMLCanvasElement) => {
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
