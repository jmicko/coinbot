...
function IncrementButtons(props: IncBtnProps) {
  ...
  const incrementButtons = buttonValues.map((value, i) => {
    ...
    return (
      <input
        ...
        onClick={(e) => { no(e); changeValue((currentValue + value).toFixed(roundTo)) }} value={`+${value}`}
      />
  ...
  const decrementButtons = buttonValues.map((value, i) => {
    return (
      <input
        ...
        onClick={(e) => { no(e); console.log(roundTo, 'roundTo'); changeValue((currentValue - value).toFixed(roundTo)) }}
        value={`-${value}`}>
